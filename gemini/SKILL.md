...
If Claude has already reviewed the same changes (e.g., via `/mentionstack:seo-analysis`
or `/mentionstack:ads-audit` earlier in the conversation), produce a cross-model
comparison:
...
- **Review FAIL:** "Gemini flagged blocking issues. Address them, then re-run `/mentionstack:gemini review`."
